...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { userId, amount, withdrawalMethod, accountDetails, notes }: WithdrawalNotification = await req.json();

    if (!userId || !amount || !withdrawalMethod || !accountDetails) {
      return new Response(
    ...
    const withdrawalMethodMap: Record<string, string> = {
      bank_transfer: "Bank Transfer",
      wire_transfer: "Wire Transfer",
      crypto: "Cryptocurrency",
    };

    const emailSubject = `[Prime Blocks] New Withdrawal Request - ${user.full_name} - $${amount.toFixed(2)}`;
    const emailBody = `
      ...
      <p><strong>Withdrawal Method:</strong> ${withdrawalMethodMap[withdrawalMethod] || withdrawalMethod}</p>
